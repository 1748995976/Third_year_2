...
{//对抽象语法树的先根遍历
  ...
  if (T)
	...
	case PARAM_LIST:    display(T->ptr[0],indent);     //依次显示全部参数类型和名称，
                        ...
                        break;
	case PARAM_DEC:     if(T->ptr[0]->type==INT)
	...
	case FOR:           printf("%*cfor循环语句：(%d)\n",indent,' ',T->pos);
                        ...
                        break;
    case FOR_DEC:       display(T->ptr[0], indent + 6);
            		    display(T->ptr[1], indent + 6);
	...
	case UMINUS:    printf("%*c%s\n",indent,' ',T->type_id);
                    ...
                    break;
    //输出数组
    case ARRAY:     display(T->ptr[0],indent);
                   // printf("%*c数组下标表达式\n",indent+3,' ');
                    display(T->ptr[1],indent+3);
                    break;
    case EXP_STRUCT: 
