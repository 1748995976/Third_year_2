...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef Specifier StructSpecifier StructName ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt StmList FORDEC DefList Def DecList Dec Exp Args 
...
%token DPLUS DMINUS LP RP LB RB LC RC SEMI COMMA DOT      /*用bison对该文件编译时，带参数-d，生成的.tab.h中给这些单词进行编码，可在lex.l中包含parser.tab.h使用这些单词种类码*/
%token ASSIGNOP_MINUS ASSIGNOP_PLUS ASSIGNOP_DIV ASSIGNOP_STAR PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN FOR SWITCH CASE COLON DEFAULT ARRAY STRUCT EXP_STRUCT
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(2,EXT_VAR_DEF,yylineno,$1,$2);}   //该结点对应一个外部变量声明
         ...
         | error SEMI   {$$=NULL;}
         ;
Specifier:  TYPE    {$$=mknode(0,TYPE,yylineno);strcpy($$->type_id,$1);if(!strcmp($1,"int")) $$->type=INT;if(!strcmp($1,"float")) $$->type=FLOAT;if(!strcmp($1,"char")) $$->type=CHAR;if(!strcmp($1,"string")) $$->type=STRING;} 
	   | StructSpecifier {}  
           ; 
/*识别结构*/
StructSpecifier: STRUCT StructName LC DefList RC   {$$=mknode(2,STRUCT_DEF,yylineno,$2,$4);}
		 |STRUCT ID  {$$=mknode(1,STRUCT_DEF,yylineno,$2);strcpy($$->type_id,$2);}  
                 ;
StructName:  {$$=NULL;}
...
VarDec:  ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
        | VarDec LB INT RB {$$=mknode(1,ARRAY_DEC,yylineno,$1);$$->type_int=$3;} 
	;
...
FORDEC:  Exp SEMI Exp SEMI Exp  {$$=mknode(3,FOR_DEC,yylineno,$1,$3,$5);}
	|SEMI Exp SEMI	Exp     {$$=mknode(2,FOR_DEC,yylineno,$2,$4);}
	;  
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | Exp PLUS PLUS {$$=mknode(1,DPLUS,yylineno,$1);strcpy($$->type_id,"DPLUS");}
      | PLUS PLUS Exp {$$=mknode(1,DPLUS,yylineno,$3);strcpy($$->type_id,"DPLUS");}
      ...
      | Exp MINUS MINUS {$$=mknode(1,DMINUS,yylineno,$1);strcpy($$->type_id,"DMINUS");}
      | MINUS MINUS Exp {$$=mknode(1,DMINUS,yylineno,$3);strcpy($$->type_id,"DMINUS");}
      ...
      /*调用结构体内部元素*/
      | Exp DOT ID    {$$=mknode(1,EXP_STRUCT,yylineno,$1);strcpy($$->type_id,$3);}
      /*识别数组*/
      | Exp LB Exp RB {$$=mknode(2,ARRAY,yylineno,$1,$3);strcpy($$->type_id,"ARRAY");}
