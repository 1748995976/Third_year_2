...
enum node_kind  { 
    ...
    ARRAY_INDEX, ARRAY_ID, ARRAY_D,
    PLUSONE_L,PLUSONE_R,MINUSONE_L,MINUSONE_R,
    FOR_DEF,FOR_CON,STRUCT_USE,STRUCT_DEF,STRUCT_DEC,COMP_STM_STRUCT,STRUCT_D,STRUCT_NAME};
...
struct ASTNode {    //以下对结点属性定义没有考虑存储效率，只是简单地列出要用到的一些属性
    ...
    struct ASTNode *ptr[3];                   //子树指针，由kind确定有多少棵子树
    int level;                    //层号
