  ...
  if (T)
	...
	case PARAM_DEC: 
            if(T->ptr[1]->ptr[0]){
                ...
                T->ptr[0]->type==INT?"int": T->ptr[0]->type==FLOAT?"float":"char",T->ptr[1]->type_id,T->ptr[1]->ptr[0]->type_int);
            }
            else{
                printf("%*c类型：%s, 参数名：%s\n", indent,' ',  \
	...
	case ARGS:      
            ...
            printf("\n");
            break;
	case ARRAY_INDEX: printf("%*c数组大小：%d \n",indent,' ',T->type_int);
	case ARRAY_ID:  
            if(T->ptr[1])printf("%*c数组名称:  %s\n",indent,' ',T->type_id);
