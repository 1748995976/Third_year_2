...
{ //显示符号表
    int i = 0;
    printf("%10s %7s %10s %12s %4s %6s\n", "变量名", "别 名", "层 号", "类  型", "标记", "偏移量");
    for (i = 0; i < symbolTable.index; i++)
    {
        if (symbolTable.symbols[i].array)
        {
            printf("%6s %6s %6d %10s %4c %6d\n", symbolTable.symbols[i].name,
                   symbolTable.symbols[i].alias, symbolTable.symbols[i].level,
                   symbolTable.symbols[i].type == INT ? "array_int" : symbolTable.symbols[i].type == FLOAT ? "array_float" : "array_char",
...
int fillSymbolTable(char *name, char *alias, int level, int type, char flag, int offset, int isArray)
{
    ...
    symbolTable.symbols[symbolTable.index].array = isArray;
...
{ //处理变量列表
    ...
    case EXT_DEC_LIST:
        ...
        break;
    case ID:
        rtn = fillSymbolTable(T->type_id, newAlias(), LEV, T->type, 'V', T->offset, 0); //最后一个变量名
        if (rtn == -1)
    ...
    if (T)
    {
        ...
        case ID: //查符号表，获得符号表中的位置，类型送type
            ...
            if (rtn == -1)
                ...
                semantic_error(T->pos, T->type_id, "函数名，类型不匹配");
            else if (symbolTable.symbols[rtn].array)
        ...
        case ARRAY:
            ...
            if (rtn == -1)
            {
                semantic_error(T->pos, T->type_id, "数组变量未定义");
            }
            else if (symbolTable.symbols[rtn].flag != 'F')
            {
                semantic_error(T->pos, T->type_id, "是函数名，类型不匹配");
            ...
            {
                semantic_error(T->pos, T->type_id, "不是数组类型");
            }
            else
            {
                T->place = rtn; //结点保存变量在符号表中的位置
        ...
        case DMIN:
        case DDIV:
            if(T->kind == DDIV && T->ptr[1] == 0)
            {
                semantic_error(T->pos,T->type_id,"除数不能为0");
            }
        case AND:   //按算术表达式方式计算布尔值，未写wan
        case OR:    //按算术表达式方式计算布尔值，未写完
        case RELOP: //按算术表达式方式计算布尔值，未写完
            if (T->ptr[0]->type != INT || T->ptr[1]->type != INT)
            {
                semantic_error(T->pos, "", "逻辑运算必须为int类型");
            }
            else
            {
                T->type = INT;
        ...
        case DIV:
            ...
            {
                //semantic_error(T->pos, T->ptr[0]->type, "shujuleixing");
                ...
                T->width = T->ptr[0]->width + T->ptr[1]->width + (T->type == INT ? 4 : (T->type==FLOAT ? 4 : 1));
            }
            break;
        case PLUSONE_L:
        case MINUSONE_L:
        ...
        case MINUSONE_R:
            /* if (T->ptr[0]->type != INT)
            {
                semantic_error(T->pos, T->type_id, "类型不是INT");
                semantic_error(T->pos,T->ptr[0], "类型不是INT");
            }*/
            //else
            //{
                 T->ptr[0]->offset = T->offset;
        ...
        case UMINUS: //未写完整
            if ((T->ptr[0]->type != INT) && (T->ptr[0]->type != FLOAT))
				semantic_error(T->pos, T->type_id, "类型不正确");
			...
			result.offset = symbolTable.symbols[T->place].offset;
			T->code = merge(2, T->ptr[0]->code, genIR(T->kind, opn1, opn2, result));
			T->width = T->ptr[0]->width + (T->type == INT ? 4 : (T->type==FLOAT ? 4 : 1));
            break;
...
{ //对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
    ...
    if (T)
    {
        ...
        case EXT_VAR_DEF: //处理外部说明,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
            ...
            T->ptr[1]->width = T->type == INT ? 4 : (T->type==FLOAT ? 4 : 1);            //将一个变量的宽度向下传递
            ext_var_list(T->ptr[1]);                              //处理外部变量说明中的标识符序列
            T->width = (T->type == INT ? 4 : (T->type==FLOAT ? 4 : 1)) * T->ptr[1]->num; //计算这个外部变量说明的宽度
        ...
        case FUNC_DEC:                                                           //根据返回类型，函数名填写符号表
            rtn = fillSymbolTable(T->type_id, newAlias(), LEV, T->type, 'F', 0, 0); //函数不在数据区中分配单元，偏移量为0
            if (rtn == -1)
            {
        ...
        case PARAM_LIST: //处理函数形式参数列表
            ...
            break;
        case PARAM_DEC:
            rtn = fillSymbolTable(T->ptr[1]->type_id, newAlias(), 1, T->ptr[0]->type, 'P', T->offset, 0);
            if (rtn == -1)
                semantic_error(T->ptr[1]->pos, T->ptr[1]->type_id, "参数名重复定义");
            else
                T->ptr[1]->place = rtn;
            T->num = 1;                                //参数个数计算的初始值
            T->width = T->ptr[0]->type == INT ? 4 : (T->ptr[0]->type ==FLOAT ? 4 : 1); //参数宽度
            result.kind = ID;
        ...
        case VAR_DEF: //处理一个局部变量定义,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
            ...
            T0->offset = T->offset;
            T->width = 0;
            width = T->ptr[1]->type == INT ? 4 : (T->ptr[1]->type==FLOAT ? 4 : 1); //一个变量宽度
            while (T0)
            { //处理所以DEC_LIST结点
                num++;
                ...
                if (T0->ptr[0]->kind == ID)
                {
                    rtn = fillSymbolTable(T0->ptr[0]->type_id, newAlias(), LEV, T0->ptr[0]->type, 'V', T->offset + T->width, 0); //此处偏移量未计算，暂时为0
                    if (rtn == -1)
                ...
                else if(T0->ptr[0]->kind == ARRAY_ID)
                {
                    ...
                    if(rtn == -1)
                    {
                        semantic_error(T0->ptr[0]->pos, T0->ptr[0]->type_id, "数组变量重复定义");
                    }
                }
                else if (T0->ptr[0]->kind == ASSIGNOP)
                {
                    rtn = fillSymbolTable(T0->ptr[0]->ptr[0]->type_id, newAlias(), LEV, T0->ptr[0]->type, 'V', T->offset + T->width, 0); //此处偏移量未计算，暂时为0
                    if (rtn == -1)
