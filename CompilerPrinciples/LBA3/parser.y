...
%token LCOM RCOM
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN PLUSONE MINUSONE MOD FOR
...
program: ExtDefList    { 
        display($1,0); semantic_Analysis0($1);
        }     /*显示语法树,语义分析 */
         ; 
ExtDefList: {$$=NULL;}
...
VarDec:  ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}    //ID结点，标识符符号串存放结点的type_id
        | ID ArrayIndex {$$=mknode(1,ARRAY_ID,yylineno,$2);strcpy($$->type_id,$1);}
         ;
ArrayIndex:  LS INT RS {$$=mknode(0,ARRAY_INDEX,yylineno);$$->type_int = $2;}
        ;
...
Stmt:   Exp SEMI    {$$=mknode(1,EXP_STMT,yylineno,$1);}
      ...
      | FOR LP Exp SEMI Exp SEMI Exp RP Stmt {$$=mknode(3,FOR,yylineno,$5,$7,$9);}
      ;
...
Dec:     VarDec  {$$=$1;}
        ...
        | VarDec ASSIGNOP ArrayDec {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}
       ;
ArrayDec:  LC Args RC    {$$=mknode(1,ARRAY_DEC,yylineno,$2);}
;
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | Exp PLUSONE   {$$=mknode(1,PLUSONE_L,yylineno,$1);strcpy($$->type_id,"PLUSONE");}
      | Exp DMUL Exp  {$$=mknode(2,DMUL,yylineno,$1,$3);strcpy($$->type_id,"DMUL");}
      | Exp DPLUS Exp {$$=mknode(2,DPLUS,yylineno,$1,$3);strcpy($$->type_id,"DPLUS");}
      | Exp DMIN Exp  {$$=mknode(2,DMIN,yylineno,$1,$3);strcpy($$->type_id,"DMIN");}
      | Exp DDIV Exp  {$$=mknode(2,DDIV,yylineno,$1,$3);strcpy($$->type_id,"DDIV");}
      | PLUSONE Exp   {$$=mknode(1,PLUSONE_R,yylineno,$2);strcpy($$->type_id,"PLUSONE");}
      | Exp MINUSONE  {$$=mknode(1,MINUSONE_L,yylineno,$1);strcpy($$->type_id,"MINUSONE");}
      | MINUSONE Exp  {$$=mknode(1,MINUSONE_R,yylineno,$2);strcpy($$->type_id,"MINUSONE");}
      ...
      | ID ArrayIndex {$$=mknode(1,ARRAY,yylineno,$2);strcpy($$->type_id,$1);}
