...
{
    int i,j;
    cache = (cache_set_t*) malloc(sizeof(cache_set_t) * S);//cache组数
    for (i=0; i<S; i++){
        cache[i]=(cache_line_t*) malloc(sizeof(cache_line_t) * E);//由E行组成的一组cache
        for (j=0; j<E; j++){
            cache[i][j].valid = 0;
            cache[i][j].tag = 0;
            cache[i][j].lru = 0;
...
void accessData(mem_addr_t addr)
{
    ...
    /*首先寻找空闲的块，若没有空闲的块就选取lru最大的块进行cache的更新*/
    int j, maxIndex = 0;
    ...
    if (j != E) {//此时j代表着空闲块
        ...
        cache_set[j].valid = 1;
        cache_set[j].tag = tag;
    } else {//没有空闲块，此时应该进行lru算法对cache进行更新
        ++eviction_count;
        for (int k = 0; k < E; ++k)
            ++cache_set[k].lru;
